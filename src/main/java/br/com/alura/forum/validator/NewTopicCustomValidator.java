...
public class NewTopicCustomValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		List<Topic> topics = topicRepository.findByOwnerAndCreationInstantAfterOrderByCreationInstantAsc(
				loggedUser, oneHourAgo);
		PossibleSpam spam = new PossibleSpam(topics);
		if(spam.hasTopicLimitExceeded()) {
			long minutesToNextTopic = spam.minutesToNextTopic(oneHourAgo);
			errors.reject("newTopicInputDto.limit.exceeded", 
					new Object[] {minutesToNextTopic}, "O limite individual de novos tópicos por hora foi excedido");
		}
	}
